...
class LogManager implements LoggerInterface
{
     ...
     *
     * @param  string  $name
     * @param  \Hlx\Log\Logger  $logger
     * @return \Hlx\Log\Logger
     */
    ...
    protected function createEmergencyLogger()
    {
        ...
        $handler = new StreamHandler(
            $config['path'] ?? $this->app->storagePath().'/logs/hlx.log',
            $this->level(['level' => 'debug'])
        );

        return new Logger(
            new Monolog('hlx', $this->prepareHandlers([$handler])),
            $this->app['events']
        );
    }
