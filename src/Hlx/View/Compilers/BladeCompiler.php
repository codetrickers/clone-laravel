...
use Hlx\Contracts\View\View;
use Hlx\Support\Arr;
use Hlx\Support\Str;
use Hlx\Support\Traits\ReflectsClosures;
use Hlx\View\Component;
...
class BladeCompiler extends Compiler implements CompilerInterface
{
     ...
     *
     * @param  \Hlx\View\Component  $component
     * @return string
     */
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                    ? "<?php if (\Hlx\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                    : "<?php if (\Hlx\Support\Facades\Blade::check('{$name}')): ?>";
        });
        ...
        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\Hlx\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\Hlx\Support\Facades\Blade::check('{$name}')): ?>";
        });
    ...
    public function aliasInclude($path, $alias = null)
    {
        ...
        $this->directive($alias, function ($expression) use ($path) {
            ...
            return "<?php echo \$__env->make('{$path}', {$expression}, \Hlx\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>";
        });
    }
