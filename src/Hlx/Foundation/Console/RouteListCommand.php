...
use Hlx\Routing\ViewController;
use Hlx\Support\Arr;
use Hlx\Support\Str;
use ReflectionClass;
...
class RouteListCommand extends Command
{
    ...
    protected function isFrameworkController(Route $route)
    {
        return in_array($route->getControllerClass(), [
            '\Hlx\Routing\RedirectController',
            '\Hlx\Routing\ViewController',
        ], true);
    }
     ...
     *
     * @param  \Hlx\Support\Collection  $routes
     * @return string
     */
    ...
    protected function formatActionForCli($route)
    {
        ...
        $rootControllerNamespace = $this->hlx[UrlGenerator::class]->getRootControllerNamespace()
            ?? ($this->hlx->getNamespace().'Http\\Controllers');
