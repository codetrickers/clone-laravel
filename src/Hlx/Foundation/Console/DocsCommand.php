...
use Hlx\Http\Client\Factory as Http;
use Hlx\Support\Arr;
...
class DocsCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Access the Hlx documentation';
     ...
     *
     * @param  \Hlx\Http\Client\Factory  $http
    ...
    {
        if ($this->isSearching()) {
            return "https://hlx.com/docs/{$this->version()}?".Arr::query([
                'q' => $this->searchQuery(),
            ]);
        }

        return with($this->page(), function ($page) {
            return trim("https://hlx.com/docs/{$this->version()}/{$page}#{$this->section($page)}", '#/');
        });
    }
    ...
    /**
     * Fetch the documentation index from the Hlx website.
     *
     * @return \Hlx\Http\Client\Response
     */
    protected function fetchDocs()
    {
        return $this->http->get("https://hlx.com/docs/{$this->version()}/index.json");
    }
    ...
    {
        return Str::before($this->version ?? $this->hlx->version(), '.').'.x';
    }
