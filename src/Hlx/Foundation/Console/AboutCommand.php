...
class AboutCommand extends Command
{
        ...
        collect(static::$data)
            ->map(fn ($items) => collect($items)
                ->map(function ($value) {
                    ...
                    if (is_string($value)) {
                        $value = $this->hlx->make($value);
                    }

                    return collect($this->hlx->call($value))
     ...
     *
     * @param  \Hlx\Support\Collection  $data
     * @return void
     */
    ...
    protected function gatherApplicationInformation()
    {
        static::addToSection('Environment', fn () => [
            ...
            'Hlx Version' => $this->hlx->version(),
            'PHP Version' => phpversion(),
            'Composer Version' => $this->composer->getVersion() ?? '<fg=yellow;options=bold>-</>',
            'Environment' => $this->hlx->environment(),
            'Debug Mode' => config('app.debug') ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
            'URL' => Str::of(config('app.url'))->replace(['http://', 'https://'], ''),
            'Maintenance Mode' => $this->hlx->isDownForMaintenance() ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
        ]);

        static::addToSection('Cache', fn () => [
            'Config' => $this->hlx->configurationIsCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Events' => $this->hlx->eventsAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Routes' => $this->hlx->routesAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Views' => $this->hasPhpFiles($this->hlx->storagePath('framework/views')) ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
        ]);
