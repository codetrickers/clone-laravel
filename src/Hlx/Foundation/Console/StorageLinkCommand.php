...
class StorageLinkCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->links() as $link => $target) {
            ...
            if (is_link($link)) {
                $this->hlx->make('files')->delete($link);
            }

            if ($relative) {
                $this->hlx->make('files')->relativeLink($target, $link);
            } else {
                $this->hlx->make('files')->link($target, $link);
            }
    ...
    {
        return $this->hlx['config']['filesystems.links'] ??
