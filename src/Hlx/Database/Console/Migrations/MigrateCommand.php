...
namespace Hlx\Database\Console\Migrations;

use Hlx\Console\ConfirmableTrait;
use Hlx\Contracts\Console\Isolatable;
use Hlx\Contracts\Events\Dispatcher;
use Hlx\Database\Events\SchemaLoaded;
use Hlx\Database\Migrations\Migrator;
use Hlx\Database\SQLiteDatabaseDoesNotExistException;
use Hlx\Database\SqlServerConnection;
use PDOException;
use Throwable;
...
class MigrateCommand extends BaseCommand implements Isolatable
{
     ...
     *
     * @var \Hlx\Database\Migrations\Migrator
     */
    protected $migrator;
     ...
     *
     * @param  \Hlx\Database\Migrations\Migrator  $migrator
    ...
    {
        if ($this->hlx['config']->get("database.connections.{$connection->getName()}.database") !== $connection->getDatabaseName()) {
            return false;
        }
        ...
        try {
            $this->hlx['config']->set("database.connections.{$connection->getName()}.database", null);
            ...
            $this->hlx['config']->set("database.connections.{$connection->getName()}.database", $connection->getDatabaseName());
        }
    }
