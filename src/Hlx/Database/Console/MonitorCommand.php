...
use Hlx\Database\Events\DatabaseBusy;
...
class MonitorCommand extends DatabaseInspectionCommand
{
     ...
     *
     * @param  \Hlx\Database\ConnectionResolverInterface  $connection
    ...
    protected function parseDatabases($databases)
    {
        return collect(explode(',', $databases))->map(function ($database) {
            if (! $database) {
                $database = $this->hlx['config']['database.default'];
            }
     ...
     *
     * @param  \Hlx\Support\Collection  $databases
     * @return void
     */
