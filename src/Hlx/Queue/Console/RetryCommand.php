...
use Hlx\Contracts\Encryption\Encrypter;
use Hlx\Queue\Events\JobRetryRequested;
use Hlx\Support\Arr;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->hlx['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->hlx['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->hlx['queue.failer']->forget($id);
            }
        }
    ...
    protected function getJobIds()
    {
        ...
        if (count($ids) === 1 && $ids[0] === 'all') {
            return Arr::pluck($this->hlx['queue.failer']->all(), 'id');
        }
    ...
    {
        $ids = collect($this->hlx['queue.failer']->all())
                        ->where('queue', $queue)
                        ->pluck('id')
                        ->toArray();
    ...
    {
        $this->hlx['queue']->connection($job->connection)->pushRaw(
    ...
    protected function refreshRetryUntil($payload)
    {
        ...
        } elseif ($this->hlx->bound(Encrypter::class)) {
            $instance = unserialize($this->hlx->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
